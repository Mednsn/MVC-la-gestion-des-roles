  ...
  <div class="relative z-10 flex min-h-screen">
    ...
    <aside class="w-64 bg-gray-900/80 text-white flex flex-col backdrop-blur-lg">
      ...
      <nav class="flex-1 px-4 space-y-2">
        ...
        <a href="#articles" class="block px-4 py-3 rounded-lg hover:bg-black/40 transition">Articles</a>
    ...
    <main class="flex-1 p-8">
      ...
      <section class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
        ...
        <div class="bg-black/40 p-6 rounded-2xl shadow hover:shadow-xl transition backdrop-blur-md">
          ...
          <p class="text-4xl font-bold text-purple-400"><?php echo count($articles); ?></p>
        </div>
        <div class="bg-black/40 p-6 rounded-2xl shadow hover:shadow-xl transition backdrop-blur-md">
      ...
      <!-- Articles Table -->
      <section id="articles" class="bg-black/40 rounded-2xl shadow p-6 backdrop-blur-md">
          ...
          <tbody>
            <?php if ($articles):
              foreach ($articles as $article): ?>
                <tr class="border-b border-gray-400 hover:bg-black/10 transition">
                  <td class="py-2"><?php echo $article->titre; ?></td>
                  <td><?php echo $article->description; ?></td>
                  <td><?php echo $article->name; ?></td>
                  <td><?php echo $article->date_creation; ?></td>
                  <td>
                    <form action="/deleteArticle" method="post">
                      <input type="hidden" name="supprimer_id" value="<?php echo $article->article_id; ?>">
                      <button type="submit" class="  rounded-xl py-1 px-3 text-bold"><i class="fa fa-trash" style="font-size:24px;color:red"></i></button>
                    </form>
                  </td>
                </tr>
